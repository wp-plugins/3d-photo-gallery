	...
	*/
	class LA_Photo_Gallery
	{
		...
		function photo_gallery_admin_options(){
			add_menu_page( '3D Photo Gallery', '3D Photo Gallery', 'manage_options', 'photo_gallery', array($this,'render_menu_page'), 'dashicons-format-image', $position );
		}
				...
				<div class="la-photo-gallery">
					...
					<div class="selected_images">
						...
						foreach ($saved_images['images'] as $key => $value) {
							echo '<div><img src="'.$value.'"><span class="dashicons dashicons-dismiss"></span><textarea placeholder="Description" name="" id="desc">'.$decs[$key].'</textarea></div>';
						}
					} ?>
					</div>
		...
		if (isset($_REQUEST['images'],$_REQUEST['des'])) {
			...
			<section id="dg-container" class="dg-container">
				<div class="dg-wrapper">
					<?php if ($saved_images['images'] != '') {
						$decs = $saved_images['des'];
				foreach ($saved_images['images'] as $key => $value) {
					echo '<a style="width:'.$saved_images['width'].'px;height:'.$saved_images['height'].'px;" ><img src="'.$value.'" style="width: 100%;height: 90%;"><div>'.$decs[$key].'</div></a>';
				}
			} ?>
				</div>
				<nav>	
